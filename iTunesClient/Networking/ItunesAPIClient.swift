...
class ItunesAPIClient {
    ...
    func searchForArtists(withTerm term: String, completion: @escaping ([Artist], ItunesError?) -> Void) {
        // for this exercize, hard-coding to music... to handle all media, have the client pass-in the media type
    ...
    func lookupArtist(with id: Int, completion: @escaping (Artist?, ItunesError?) -> Void) {
        ...
        performRequest(with: endpoint) { results, error in
            ...
            guard let artistInfo = results.first else {
                completion(nil, .jsonParsingFailure(message: "Result does not contain artist info"))
                return
            }
